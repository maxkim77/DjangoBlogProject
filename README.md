...
- 블로그 포스팅: 글을 작성하고 게시 가능
- 댓글 기능: 각 포스트에 댓글을 작성하고 관리 가능
- 태그 및 카테고리 분류: 포스트를 태그와 카테고리로 검색 가능
...
- 공지사항 기능
관리자만 관리자페이지에서 설정 가능
model로 is_notice를 설정하고, views.py에서 get_context_data 메서드에서 공지사항 파트와 일반 게시글 파트로 두 변수를 나눔.
